...
class RealBeachDetector:
    def __init__(self):
        ...
        # 현재 스크립트 위치 기준으로 비디오 파일 경로 설정
        script_dir = os.path.dirname(os.path.abspath(__file__))
        backend_videos_dir = os.path.join(script_dir, "..", "backend", "videos")
        ...
        self.beaches = [
            {
                ...
                "video_path": os.path.join(backend_videos_dir, "hamduck_beach.mp4")
            },
            {
                "name": "Iho Beach", 
                "source": "iho_camera_01",
                "video_path": os.path.join(backend_videos_dir, "iho_beach.mp4")
            },
            {
                "name": "Walljeonglee Beach", 
                "source": "walljeonglee_camera_01",
                "video_path": os.path.join(backend_videos_dir, "walljeonglee_beach.mp4")
            }
        ]
        
        # YOLO 설정
        self.yolo_weights = os.getenv("YOLO_WEIGHTS", os.path.join(script_dir, "yolov8n.pt"))
        self.yolo_conf = float(os.getenv("YOLO_CONF", "0.35"))
        self.yolo_iou = float(os.getenv("YOLO_IOU", "0.5"))
        self.yolo_imgsz = int(os.getenv("YOLO_IMGSZ", "640"))  # 성능 향상을 위해 640으로 조정
...
def main():
    ...
    try:
        ...
        print("[INFO] 문제 해결 방법:")
        print("   1. Python 환경 확인: python --version")
